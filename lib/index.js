...
import regObj,{verifyFn} from './reg-list'
...
export const valid = {
  bind:(el,{ value, modifiers })=>{ 
    ...
    if(FORN_ITEM.indexOf(el.nodeName.toLocaleLowerCase())==-1) {
      console.warn("The element of " + el.nodeName + " doesn't  use this directive");
      return 
    }
    ...
    if(isObject(value)){
      ...
      //get regExp 
       reg = isReg(value.reg) ? value.reg : regObj[regType]; 
    } else{
      regType = value;
      reg=regObj[regType]
    } 
    ...
    if(INPUT_EVENT.indexOf(triggerEvent)!=-1){   
     ...
     el.addEventListener(triggerEvent,function(ev){  
